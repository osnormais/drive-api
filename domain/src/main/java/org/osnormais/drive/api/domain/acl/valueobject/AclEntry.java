...
public record AclEntry(
        ...
        Instant grantedAt,
        Optional<Instant> expiresAt) implements ValueObject {

    public static AclEntry grant(final UserId user, final Permission permission) {
        return new AclEntry(user, permission, Instant.now(), Optional.empty());
    }

    public static AclEntry grantUntil(final UserId user, final Permission permission, final Instant expiresAt) {
        return new AclEntry(user, permission, Instant.now(), Optional.ofNullable(expiresAt));
    }
    ...
    public void validate(final ValidationHandler handler) {
        ...
        if (isNull(expiresAt))
            handler.append(new ValidationError("'AclEntry.expiresAt' cannot be null."));

    }

    public Boolean isActive() {
        return expiresAt.isEmpty() || expiresAt.get().isAfter(Instant.now());
    }
