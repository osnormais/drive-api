...
public class FilePublicationFailedEvent extends DomainEvent<FileId> {

    private static final Class<? extends Entity<?>> ENTITY_CLASS = File.class;
    private static final String SUB_RESOURCE = "publication";
    private static final String ACTION = "failed";

    FilePublicationFailedEvent() {
    }

    private FilePublicationFailedEvent(
            final File file,
    ...
    public static FilePublicationFailedEvent create(final File file) {
        return new FilePublicationFailedEvent(
                file,
                Instant.now(),
                List.of(
